# H.py - COMPLETE SILENT AUTO-RESTART VERSION (NO OWNER NOTIFICATIONS)
import telebot
import subprocess
import os
import zipfile
...
bot_locked = False
server_running = True  # ✅ हमेशा RUNNING रहेगा
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('🏥 Check Health', callback_data='check_health'),
            types.InlineKeyboardButton('🔄 Restart Dead', callback_data='restart_dead')
        ]
        
        # Arrange buttons in rows
        markup.add(buttons[0])  # Updates Channel
        markup.add(buttons[1], buttons[2])  # Upload File, Check Files
        markup.add(buttons[3], admin_buttons[0])  # Bot Speed, Subscriptions
        markup.add(buttons[4], admin_buttons[1])  # Statistics, Broadcast
        markup.add(admin_buttons[2], admin_buttons[3])  # Lock Bot, Admin Panel
        markup.add(admin_buttons[4], admin_buttons[5])  # Start Server, Stop Server
        markup.add(admin_buttons[6], admin_buttons[7])  # Restart Server, Check Health
        markup.add(admin_buttons[8])  # Restart Dead
        markup.add(buttons[5], buttons[6])  # Contact Owner, MPX AI
        markup.add(buttons[7])  # Uptime
    else:
...
def start_all_server_scripts(message_obj_for_reply):
    ...
    logger.info(f"Starting all server scripts")
    ...
    if message_obj_for_reply:
        ...
        logger.info(f"Auto-started {started_count}/{total_files} scripts for {len(all_user_files_snapshot)} users")
...
def stop_all_server_scripts(message_obj_for_reply):
    ...
    logger.info(f"Stopping all server scripts")
    ...
    if message_obj_for_reply:
        ...
        logger.info(f"Auto-stopped {stopped_count} scripts")
...
def restart_all_server_scripts(message_obj_for_reply):
    ...
    logger.info(f"Restarting all server scripts")
    ...
    if message_obj_for_reply:
        ...
        logger.info(f"Auto-restarted {started_count}/{total_files} scripts for {len(all_user_files_snapshot)} users")

# ==================== AUTO-RESTART SYSTEM ====================

def monitor_and_auto_restart():
    """
    मॉनिटर थ्रेड जो हर 60 सेकंड में सभी स्क्रिप्ट्स की स्थिति चेक करे
    और बंद हुई स्क्रिप्ट्स को ऑटोमैटिक रीस्टार्ट करे
    """
    # पहला चेक 30 सेकंड बाद
    time.sleep(30)
    
    while True:
        try:
            # ✅ हर 60 सेकंड में चेक करे
            time.sleep(60)
                
            logger.info("🔍 Auto-restart monitor: Checking all scripts...")
            ...
            for target_user_id, files_for_user in all_user_files_snapshot.items():
                ...
                for file_name, file_type in files_for_user:
                    ...
                    if not os.path.exists(file_path):
                        logger.warning(f"Auto-restart: File {file_name} missing for user {target_user_id}. Skipping.")
                        continue
                    
                    # स्क्रिप्ट चल रही है या नहीं
                    is_running = is_bot_running(target_user_id, file_name)
                    
                    if not is_running:
                        logger.warning(f"⚠️ Script {file_name} (User: {target_user_id}) is NOT running. Attempting auto-restart...")
                        
                        try:
                            # क्रिएट डमी मैसेज ऑब्जेक्ट
                            class DummyMessage:
                            ...
                            dummy_msg = DummyMessage(chat_id=target_user_id, user_id=target_user_id)
                            
                            if file_type == 'py':
                                threading.Thread(target=run_script, 
                                               args=(file_path, target_user_id, user_folder, file_name, dummy_msg),
                                               daemon=True).start()
                            elif file_type == 'js':
                            ...
                            restarted_count += 1
                            logger.info(f"✅ Auto-restarted: {file_name} for user {target_user_id}")
                            
                            # ⚠️ OWNER को NO NOTIFICATION (साइलेंट मोड)
                            # कोई भी नोटिफिकेशन नहीं भेजेगा
                            
                            # थोड़ा वेट करें
                            time.sleep(2)
                            
                        except Exception as restart_err:
                            logger.error(f"❌ Failed to auto-restart {file_name} for {target_user_id}: {restart_err}")
                            logger.error(traceback.format_exc())
            
            if restarted_count > 0:
                logger.info(f"Auto-restart completed. Restarted {restarted_count} scripts.")
                
        except Exception as monitor_err:
            logger.error(f"Error in auto-restart monitor: {monitor_err}")
            logger.error(traceback.format_exc())
            time.sleep(300)  # एरर आने पर 5 मिनट wait

def start_auto_restart_monitor():
    """ऑटो-रेस्टार्ट मॉनिटर थ्रेड शुरू करें"""
    monitor_thread = threading.Thread(target=monitor_and_auto_restart, daemon=True)
    monitor_thread.start()
    logger.info("✅ Auto-restart monitor thread started")

def check_script_health(script_owner_id, file_name):
    """
    स्क्रिप्ट की हेल्थ डिटेल्ड चेक करें
    """
    script_key = f"{script_owner_id}_{file_name}"
    script_info = bot_scripts.get(script_key)
    ...
    try:
        ...
        memory_mb = memory_info.rss / 1024 / 1024  # MB में
        ...
        return {
            ...
            'create_time': datetime.fromtimestamp(create_time).strftime('%H:%M:%S')
        }
...
def log_script_status():
    """सभी स्क्रिप्ट्स का स्टेटस लॉग करें"""
    logger.info("=" * 50)
    logger.info("SCRIPT STATUS REPORT")
    logger.info("=" * 50)
    ...
    for script_key, script_info in bot_scripts.items():
        script_owner_id, file_name = script_key.split('_', 1)
        is_running = is_bot_running(int(script_owner_id), file_name)
        
        if is_running:
            running_count += 1
            health = check_script_health(int(script_owner_id), file_name)
            logger.info(f"🟢 {file_name} (User: {script_owner_id}) - Running - Health: {health}")
        else:
            logger.info(f"🔴 {file_name} (User: {script_owner_id}) - Stopped")
    
    logger.info(f"Total: {total_scripts}, Running: {running_count}, Stopped: {total_scripts - running_count}")
    logger.info("=" * 50)

def periodic_status_log():
    """हर 30 मिनट में स्टेटस लॉग करें"""
    while True:
...
def start_periodic_logging():
    """पीरियोडिक लॉगिंग थ्रेड शुरू करें"""
    log_thread = threading.Thread(target=periodic_status_log, daemon=True)
    log_thread.start()
    logger.info("✅ Periodic status logging started")
...
def _logic_send_welcome(message):
    ...
    if user_id not in active_users:
        ...
        # ⚠️ NO OWNER NOTIFICATION (साइलेंट मोड)
    ...
    welcome_msg_text = (f"Welcome, {user_name}!\n\nYour User ID: `{user_id}`\n"
                        ...
                        f"🔄 **AUTO-RESTART:** Scripts auto-restart if stopped!\n\n"
...
def _logic_upload_file(message):
    ...
    bot.reply_to(message, 
                ...
                "🔄 **Auto-restart enabled for all scripts!**")
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        speed_msg = (f"Bot Speed & Status:\n\nAPI Response Time: {response_time} ms\n"
                     ...
                     f"Auto-Restart: {'🟢 ACTIVE' if server_running else '🔴 INACTIVE'}")
...
def _logic_uptime(message):
    ...
    bot.reply_to(message, f"Bot Uptime: `{uptime_str}`\nServer Status: {'🟢 RUNNING' if server_running else '🔴 STOPPED'}\nAuto-Restart: {'🟢 ACTIVE' if server_running else '🔴 INACTIVE'}", parse_mode='Markdown')
...
def _logic_statistics(message):
    ...
    if user_id in admin_ids:
        ...
        stats_msg += f"Total Stopped Scripts: {total_files_records - running_bots_count}"
    else:
        stats_msg += f"\n👤 **Your Stats:**\n"
...
def _logic_check_health(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "Admin permissions required.")
        return
    
    response = "🏥 **Script Health Report**\n\n"
    
    if not bot_scripts:
        response += "No scripts running."
    ...
    for script_key, script_info in bot_scripts.items():
        ...
        if is_running:
            ...
            if isinstance(health, dict):
                response += (f"🟢 `{file_name}` (User: `{script_owner_id}`)\n"
                           f"   PID: {health['pid']} | CPU: {health['cpu_percent']}% | "
                           f"Mem: {health['memory_mb']}MB | Uptime: {health['runtime_seconds']}s\n")
            else:
                response += f"🟡 `{file_name}` (User: `{script_owner_id}`) - {health}\n"
        else:
            response += f"🔴 `{file_name}` (User: `{script_owner_id}`) - STOPPED\n"
        
        response += "\n"
    
    bot.reply_to(message, response, parse_mode='Markdown')

def _logic_restart_dead(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "Admin permissions required.")
        return
    
    bot.reply_to(message, "🔍 Checking for dead scripts...")
            ...
            if not is_bot_running(target_user_id, file_name):
                ...
                if os.path.exists(file_path):
                    try:
                        if file_type == 'py':
                            threading.Thread(target=run_script, 
                                           args=(file_path, target_user_id, user_folder, file_name, message)).start()
                        elif file_type == 'js':
                            threading.Thread(target=run_js_script,
                                           args=(file_path, target_user_id, user_folder, file_name, message)).start()
                        ...
                        logger.error(f"Error restarting {file_name}: {e}")
    
    bot.reply_to(message, f"✅ Restarted {restarted} dead scripts.")
...
def command_server_status(message):
    ...
    status_msg = (f"📊 **Server Status:**\n\n"
                  ...
                  f"🔴 Stopped Scripts: {len(bot_scripts) - running_count}\n"
    ...
    if user_id == OWNER_ID:
        ...
        status_msg += f"Use /checkhealth for detailed health"
...
def ping(message):
    ...
    uptime_str = get_uptime()
    bot.edit_message_text(f"Pong!\nLatency: {latency} ms\nUptime: {uptime_str}\nServer: {'🟢 RUNNING' if server_running else '🔴 STOPPED'}\nAuto-Restart: {'🟢 ACTIVE' if server_running else '🔴 INACTIVE'}",
                          message.chat.id, msg.message_id)

@bot.message_handler(commands=['checkhealth'])
def command_check_health(message):
    _logic_check_health(message)

@bot.message_handler(commands=['restartdead'])
def command_restart_dead(message):
    _logic_restart_dead(message)

@bot.message_handler(commands=['scriptinfo'])
def command_script_info(message):
    """विशेष स्क्रिप्ट की डिटेल्ड जानकारी"""
    if message.from_user.id not in admin_ids:
    ...
    if len(args) < 3:
        bot.reply_to(message, "Usage: /scriptinfo <user_id> <filename>\nExample: /scriptinfo 12345678 bot.py")
        return
    ...
    try:
        ...
        is_running = is_bot_running(user_id, filename)
        
        response = f"📊 **Script Info**\n\n"
        response += f"File: `{filename}`\n"
        response += f"User: `{user_id}`\n"
        response += f"Status: {'🟢 RUNNING' if is_running else '🔴 STOPPED'}\n"
        
        if is_running:
            health = check_script_health(user_id, filename)
            if isinstance(health, dict):
                response += f"\n**Health Details:**\n"
                response += f"PID: `{health['pid']}`\n"
                response += f"Status: `{health['status_str']}`\n"
                response += f"CPU: `{health['cpu_percent']}%`\n"
                response += f"Memory: `{health['memory_mb']} MB`\n"
                response += f"Uptime: `{health['runtime_seconds']} seconds`\n"
                response += f"Started: `{health['create_time']}`\n"
            else:
                response += f"\nHealth Check: `{health}`\n"
        
        # लॉग फाइल साइज
        user_folder = get_user_folder(user_id)
        log_path = os.path.join(user_folder, f"{os.path.splitext(filename)[0]}.log")
        if os.path.exists(log_path):
            size_kb = os.path.getsize(log_path) / 1024
            response += f"\nLog Size: `{size_kb:.2f} KB`"
        ...
        logger.error(f"Error in scriptinfo: {e}")
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "🏥 Check Health": _logic_check_health,
    "🔄 Restart Dead": _logic_restart_dead
}
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'check_health':
            admin_required_callback(call, check_health_callback)
        elif data == 'restart_dead':
            admin_required_callback(call, restart_dead_callback)
        elif data == 'broadcast':
    ...
    try:
        ...
        try:
            bot.edit_message_text(
                ...
                f"🔄 Auto-Restart: {'🟢 ENABLED' if server_running else '🔴 DISABLED'}",
...
def back_to_main_callback(call):
    ...
    main_menu_text = (f"Welcome back, {call.from_user.first_name}!\n\n"
                      ...
                      f"🚀 Server: {'🟢 RUNNING' if server_running else '🔴 STOPPED'}\n"
                      f"🔄 Auto-Restart: {'🟢 ACTIVE' if server_running else '🔴 INACTIVE'}{admin_info}\n\n"
                      f"Use buttons or type commands.")
    try:
        bot.answer_callback_query(call.id)
...
def check_health_callback(call):
    bot.answer_callback_query(call.id)
    _logic_check_health(call.message)
    try:
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id,
                                      reply_markup=create_main_menu_inline(call.from_user.id))
    except Exception as e:
        logger.error(f"Error updating menu after check_health: {e}")

def restart_dead_callback(call):
    bot.answer_callback_query(call.id)
    _logic_restart_dead(call.message)
    try:
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id,
                                      reply_markup=create_main_menu_inline(call.from_user.id))
    except Exception as e:
        logger.error(f"Error updating menu after restart_dead: {e}")
...
if __name__ == '__main__':
    ...
    # ✅ ऑटो-रेस्टार्ट सिस्टम शुरू करें
    start_auto_restart_monitor()
    
    # ✅ पीरियोडिक लॉगिंग शुरू करें
    start_periodic_logging()
    
    # ✅ सभी एक्सिस्टिंग स्क्रिप्ट्स को ऑटो स्टार्ट करें
    logger.info("Auto-starting existing scripts...")
    time.sleep(2)
    
    # ✅ साइलेंट मोड - Owner को कोई नोटिफिकेशन नहीं
    # सभी स्क्रिप्ट्स ऑटो स्टार्ट करें (बिना किसी मैसेज के)
    if len(user_files) > 0:
        logger.info(f"Found {len(user_files)} users with scripts. Starting all SILENTLY...")
        start_all_server_scripts(None)  # None = कोई मैसेज नहीं भेजेगा
    
    logger.info("✅ Bot is now fully automatic. No owner intervention needed!")
    logger.info("✅ All scripts will auto-start and auto-restart 24/7")
